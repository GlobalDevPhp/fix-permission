...
wp-content/cache/index.html'; ?></textarea>
            <textarea rows="10" cols="30" id="status_area" disabled="disabled"><?php echo $statuses; ?></textarea>
            <textarea rows="10" cols="30" id="logs_area" wrap="off" disabled="disabled"><?php echo $logs; ?></textarea>
        </div>            
        <div>
        ...
        <div>
            <p></p>
            <button type="button" class="button button-primary btn-ajtest" data-nonce="<?php echo esc_attr(wp_create_nonce('fix-permission_revisions')); ?>"><?php esc_html_e('Run action', 'fix-permission'); ?></button>
        </div>            
    </form>
</div>
<style>
            ...
            $.post(ajaxurl, 
                {
                    ...
                    fperm_options_paths: $('textarea[name="fperm_options_paths"]').val(),
                    action_type: $('select[name="action_type"]').val(),                    
                    permission_flag: $('select[name="permission_flag"]').val(),
                    test_mode: $('select[name="test_mode"]').val(),
                    recursion_on: $('#recursion_on').is(":checked")?'yes':'no',
                    '_wpnonce': $node.data('nonce'),
                    generated_nonce : $('#generated_nonce').val(),
                }, function (req) {
                if (req.success) {
                    $('textarea[name="fperm_options_paths"]').val(req.data.paths);
                    $('#status_area').val(req.data.statuses);
                    $('#logs_area').val(req.data.logs);
                }
            });
        });
